...
# 添加表字段
try:
    cursor.execute("ALTER TABLE portal_message_channel_dingtalk ADD COLUMN dingtalk_type VARCHAR(20) NOT NULL DEFAULT 'internal' COMMENT '对接方式' AFTER channel_id")
    conn.commit()
    print('portal_message_channel_dingtalk 添加 dingtalk_type 成功')
...
```python
# Python脚本中检查
cursor.execute("SHOW TABLES LIKE 'portal_message_template'")
exists = cursor.fetchone() is not None

cursor.execute("SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'ry392' AND TABLE_NAME = 'portal_message_channel_dingtalk' AND COLUMN_NAME = 'dingtalk_type'")
...
```python
# 幂等添加字段（如果字段已存在会报错，需要用try-catch处理）
try:
    cursor.execute("ALTER TABLE {table_name} ADD COLUMN {column_name} {type} COMMENT '{comment}' AFTER {after_column}")
    conn.commit()
    print('添加成功')
except Exception as e:
...
cursor.execute("""SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
    ...
    (DB_DATABASE, 'portal_message_channel_dingtalk', 'dingtalk_type'))
...
fields_to_add = [
    ("portal_message_channel_dingtalk", "dingtalk_type", "VARCHAR(20) NOT NULL DEFAULT 'internal' COMMENT '对接方式' AFTER channel_id"),
    ("portal_message_channel_wecom", "wecom_type", "VARCHAR(20) NOT NULL DEFAULT 'internal' COMMENT '对接方式' AFTER channel_id"),
    ("portal_message_channel_feishu", "feishu_type", "VARCHAR(20) NOT NULL DEFAULT 'internal' COMMENT '对接方式' AFTER channel_id"),
]

for table, col, definition in fields_to_add:
    try:
...
| 简单查询 | 直接SQL |
| 多SQL升级脚本 | `--file` |
| 添加表字段（幂等） | `--python` + Python脚本 |
| 检查并添加字段 | `--python` + Python脚本（带IF判断）|

### 5.1 添加字段的正确流程

1. 先查询现有表结构：`DESCRIBE table_name`
2. 编写Python脚本添加字段（带try-catch）
3. 执行：`python mysql_query.py --python add_columns.py`
4. 验证：`DESCRIBE table_name` 确认字段已添加
5. 更新 `portal_init.sql` 中的建表语句
...
| 字段已存在 | 重复执行ADD COLUMN | 使用IF判断或try-catch |
...
### 变更历史
- v1.3.0 (2026-04-16): 新增ALTER TABLE添加字段的Python脚本模板和最佳实践
